...
RSpec.describe Practice::RenewController, type: :controller do
  ...
  before(:each) do
    ...
    allow_any_instance_of(GetDomainExpireDate).to receive(:process).with(domain: 'test.com',
                                                                         api_connector: @api_connector).and_return(true)
    allow_any_instance_of(GetDomainExpireDate).to receive(:process).with(domain: 'awesome.com',
                                                                         api_connector: @api_connector).and_return(true)
